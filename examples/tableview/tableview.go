...
func main() {
	...
	canvas.GradientFillRectangle(wcolor.RGB(255, 0, 0), wcolor.RGB(0, 255, 0), walk.Horizontal, walk.Rectangle{0, 0, 100, 1})
		...
		Children: []Widget{
			...
			TableView{
				...
				StyleCell: func(style *walk.CellStyle) {
					...
					if item.checked {
						if style.Row()%2 == 0 {
							style.BackgroundColor = wcolor.RGB(159, 215, 255)
						} else {
							style.BackgroundColor = wcolor.RGB(143, 199, 239)
						}
					}
					...
					case 2:
						if item.Baz >= 900.0 {
							style.TextColor = wcolor.RGB(0, 191, 0)
							style.Image = goodIcon
						} else if item.Baz < 100.0 {
							style.TextColor = wcolor.RGB(255, 0, 0)
							style.Image = badIcon
						}
